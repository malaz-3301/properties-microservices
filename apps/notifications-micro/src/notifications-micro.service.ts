...
export class NotificationsMicroService {
  ...
  async sendNotificationForAllSidesInProperties(
    ...
    const ownerMessage =  this.i18nService.t(`transolation.${message}`, {
      lang: owner.language,
    });
    const ownerNotification = this.notificationMicro.create({
      property: contract.property,
      user: contract.user,
      usre_language_message: `${ownerMessage} ${contract.expireIn}`,
    });
    await this.notificationMicro.save(ownerNotification);
    const userMessage = this.i18nService.t(`transolation.${message}`, {
      lang: user.language,
    });
    const userNotification = this.notificationMicro.create({
      property: contract.property,
      user: contract.property.owner,
      usre_language_message: `${userMessage} ${contract.expireIn}`,
    });
    await this.notificationMicro.save(userNotification);
    const agencyMessage =this.i18nService.t(`transolation.${message}`, {
      lang: agency.language,
    });
    const agencyNotification = this.notificationMicro.create({
      property: contract.property,
      user: contract.agency,
      usre_language_message: `${agencyMessage} ${contract.expireIn}`,
    });
    await this.notificationMicro.save(agencyNotification);
  }
}
